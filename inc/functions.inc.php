...
function login() {
    ...
    if ($student_no != "" && $email != "") {
		...
		if ($student_no == ADMIN_USER && $email == ADMIN_PASSWORD) {
			$_SESSION['student_no'] = ADMIN_USER;
		}
		else {
...
function is_admin() {
    ...
    return ($_SESSION['student_no'] == ADMIN_USER);
}
...
function list_students() {
    ...
    // and update student_list.tpl to display the result
    
	global $mysql;

    $query = "SELECT * FROM dat310.students";
	$students = array();	
	if ($result = mysqli_query($mysql, $query)) {	
        while ($student = mysqli_fetch_assoc($result)) {
			$students[] = $student;
			
			//echo "Adding new DB record";
			//print_r($student);
		}
	}
	//echo "All records";
